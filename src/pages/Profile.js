...
const Profile = () => {
    ...
    const closeNotif = (e,notif) => {
        ...
        instance.post(
            `/profile/notifs/${notif.requestid}`
        ).then( res => {
            console.log(res.data)
    }).catch(
        err => alert(err))
    ...
    const createReqData = (reqData) => {
        if (reqData[5] === 'approved') {
            ...
            <div className="card h-100">
                    ...
                    <p className="card-text">You approved {reqData[1]}'s request on {reqData[0]}</p>
            ...
            return (
                <div className="card h-100">
                    <div className="card-body">
                        ...
                        <a href="#" class="btn btn-outline-success" onClick={e => approve(e,reqData)}>Approve</a>
                        <a href="#" class="btn btn-outline-danger" onClick={e => decline(e,reqData)}>Decline</a>
                </div>
        )}}
    ...
    const createNotifs = (notif) => {
        ...
        return (
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                Your request on <strong>{notif.title}</strong> by <strong>{notif.username}</strong> was declined
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"
                 onClick={e => closeNotif(e,notif) }></button>
            </div>
        )
        } else if (notif.status === "approved") {
            return (
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                Your request on <strong>{notif.title}</strong> by <strong>{notif.username}</strong> was accepted! Check out the <b>Your Requests</b> page
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"
                 onClick={e => closeNotif(e,notif) }></button>
            </div>
            )
    ...
    return (
        <>
            ...
            <div className='profile-background'>
                ...
                { notifs &&
                       notifs.map(item => createNotifs(item))
                }   
                </div>
                <div style={{color: '#b3e0ff'}}>
                    ...
